...
const CadastrarRota = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: `Erro ao criar rota: ${error.message}` });
  }
};
...
const SelecionarRota = async (req, res) => {
  ...
  try {
    ...
    if (tipo !== 'admin' && rota.empresa_id !== empresa_id) {
      return res.status(403).json({ message: 'Você só pode visualizar rotas da sua própria empresa.' });
    }

    res.status(200).json(rota);
  } catch (error) {
    console.error('Erro ao selecionar rota:', error);
    res.status(500).json({ message: 'Erro ao selecionar rota' });
  }
};
...
const AtualizarRota = async (req, res) => {
  ...
  try {
    ...
    if (tipo !== 'admin' && rota.empresa_id !== empresa_id) {
      return res.status(403).json({ message: 'Você só pode atualizar rotas da sua própria empresa.' });
    }
...
const DeletarRota = async (req, res) => {
  ...
  try {
    ...
    if (tipo !== 'admin' && rota.empresa_id !== empresa_id) {
      return res.status(403).json({ message: 'Você só pode deletar rotas da sua própria empresa.' });
    }
...
const BuscarRotasPorEmpresaId = async (req, res) => {
  ...
  try {
    ...
    if (rotas.length > 0) {
      ...
      res.status(404).json({ message: 'Nenhuma rota encontrada para esta empresa' });
    }
  } catch (error) {
